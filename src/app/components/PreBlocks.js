...
export function PreBlock11() {
  ...
  25% - 전체즉사 5초마다 총 4번 발동
        ...
        데미지가 쌔면 50%기둥파괴시 보스스턴 시간동안 패턴스킵 가능
`;
  return (
    <pre>{pres}</pre>
  );
...
export function PreBlock12() {
  const pres =`
설날,신년 이벤트가 종료되며, 봄맞이 이벤트가 시작됩니다.
...
봄맞이 이벤트 기간동안 진행되며, 출석체크시 랜덤한 추가 보상이 지급됩니다.
1. 2~4성 랜덤 각인
2. 100 ~ 1000 골드
3. 1,000,000 ~ 10,000,000 은화
4. 봄맞이 코인

시련의 탑 진행 시 간헐적 발생하던 점수관련 오류가 수정됩니다.

어빌리티 프리셋 슬롯이 1개 더 추가됩니다.

태초 등급의 어빌리티 능력이 추가 됩니다.
유니크 : 72.4% 
레전더리 : 22.5% 
에픽 : 4.3% 
신화 : 0.7%
...
태초 아바타
*조합 : 신화 아바타 + 태초 옷감 10개
공격력 +45.5%, 공격속도 +45.5%, 치명타 확률 +14.3%, 추가피해 +19.5%, 세트점수 +500

태초 옷감은 신화 옷감 10개로 교환 할 수 있습니다.
...
전용템 강화 안전단계가 추가됩니다.
*8강, 11강, 15강
강화 안전단계에서는 강화에 실패해도 강화단계가 하락하지 않습니다.

1~7티어 레이드의 드랍률이 상향 조정됩니다.

7,8티어 레이드의 난이도 및 능력치가 하향 조정됩니다.

명예 패키지의 보상이 상향 조정됩니다.
1단계 명예 코인 총 획득량 1개 -> 2개
2단계 명예 코인 총 획득량 3개 -> 5개
3단계 명예 코인 총 획득량 5개 -> 8개
